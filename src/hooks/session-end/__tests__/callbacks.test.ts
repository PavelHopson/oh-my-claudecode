...
describe('interpolatePath', () => {
  ...
  it('replaces ~ with homedir', () => {
    ...
    expect(result.replaceAll('\\', '/')).toContain('/logs/test.md');
  });
...
describe('triggerStopCallbacks', () => {
  ...
  it('writes file when file callback is enabled', async () => {
    ...
    const outputPath = normalize('/tmp/test-test-session-123.md');
    expect(mockMkdirSync).toHaveBeenCalledWith(dirname(outputPath), { recursive: true });
